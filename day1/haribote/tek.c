...
int tek_getsize(unsigned char *p) {
    ...
    int size = -1;
    if (mymemcmp(p + 1, header, 15) == 0 && (*p == 0x83 || *p == 0x85 || *p == 0x89)) {
        p += 16;
...
static int tek_decmain5(int *work, UCHAR *src, int osiz, UCHAR *q, int lc, int pb, int lp, int flags) {
    ...
    if (stk) {
        ...
        */
        rd->rmsk |= -1;
        rd->bm[1].t = 5;
        rd->bm[1].m = 3; /* for fchgprm */
        rd->bm[2].t = 9;
        rd->bm[2].m = 2; /* for tbmt, tbmm */
        ...
        if (tek_rdget1(rd, &prb->pb[s_pos].st[s].mch, 0x71, 0, rd->ptbm[s > 0]) ^ stk) { /* 非lz */
            ...
            if (tek_rdget1(rd, &prb->st[s].rep, 0x71, 0, rd->ptbm[13]) ^ stk) { /* len/dis */
                ...
                if (j >= 4) {
                    ...
                    if (j < 14) /* k < 6 */
                        rep[0] |= tek_revbit(
                                tek_rdget1(rd, &prb->spdis[j & 1][(1 << k) - 2], k | 0x70, 1, rd->ptbm[10 + (k >= 4)]),
                                k);
                    else {
                        if (stk == 0) {
                            if (k -= 6)
