...
api_initmalloc: ; void api_initmalloc(void);
    PUSH    EBX
    MOV     EDX,8
    MOV     EBX,[CS:0x0060] ; malloc 領域の番地
    MOV     EAX,EBX
...
api_inittimer: ; int api_inittimer(int timer, int data);
    PUSH    EBX
    MOV     EDX,17
    MOV     EBX,[ESP+8]     ; timer
    MOV     EAX,[ESP+12]    ; data
...
api_settimer: ; int api_settimer(int timer, int time);
    PUSH    EBX
    MOV     EDX,18
    MOV     EBX,[ESP+8]     ; timer
    MOV     EAX,[ESP+12]    ; time
...
api_freetimer: ; int api_freetimer(int timer);
    PUSH    EBX
    MOV     EDX,19
    MOV     EBX,[ESP+8]     ; timer
    INT     0x40
